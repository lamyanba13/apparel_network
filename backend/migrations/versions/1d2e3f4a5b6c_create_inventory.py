...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("active", "out_of_stock", "discontinued", native_enum=False),
            server_default="active",
            nullable=False,
        ),
        sa.Column(
            "tracking_policy",
            sa.Enum("track", "do_not_track", "preorder", native_enum=False),
            server_default="track",
            nullable=False,
        ),
        sa.Column(
            "low_stock_threshold", sa.Integer(), server_default="0", nullable=False
        ),
        sa.Column(
            "created_at",
        ...
        sa.UniqueConstraint("variant_id", name="uq_inventory_items_variant"),
        sa.CheckConstraint(
            "quantity_on_hand >= 0", name="inventory_on_hand_nonnegative"
        ),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint("version >= 1", name="inventory_version_positive"),
    )
    for name, columns in (
        ("ix_inventory_items_product", ["product_id"]),
        ("ix_inventory_items_catalog", ["catalog_id"]),
        ("ix_inventory_items_store", ["store_id"]),
        ("ix_inventory_items_status", ["status"]),
        ("ix_inventory_items_available", ["quantity_available"]),
        ("ix_inventory_items_deleted", ["deleted_at"]),
    ):
...
def downgrade() -> None:
    for name in (
        "ix_inventory_items_deleted",
        "ix_inventory_items_available",
        "ix_inventory_items_status",
        "ix_inventory_items_store",
