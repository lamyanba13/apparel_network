...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "media_type", sa.Enum("image", "video", native_enum=False), nullable=False
        ),
        sa.Column(
            "role",
            sa.Enum("primary", "gallery", "thumbnail", "document", native_enum=False),
            nullable=False,
        ),
        sa.Column(
            "storage_provider", sa.String(32), server_default="minio", nullable=False
        ),
        ...
        sa.UniqueConstraint("object_key", name="uq_product_media_object_key"),
        sa.CheckConstraint("file_size > 0", name="product_media_file_size_positive"),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint("version >= 1", name="product_media_version_positive"),
        sa.CheckConstraint(
    ...
    for name, columns in (
        ("ix_product_media_product", ["product_id"]),
        ("ix_product_media_store", ["store_id"]),
        ("ix_product_media_catalog", ["catalog_id"]),
        ("ix_product_media_product_role", ["product_id", "role"]),
        ("ix_product_media_product_order", ["product_id", "display_order"]),
    ):
    ...
    op.create_index(
        ...
        postgresql_where=sa.text(
            "role = 'primary' AND media_type = 'image' AND is_active = true AND "
            "deleted_at IS NULL"
        ),
    )
    op.create_index(
...
def downgrade() -> None:
    for name in (
        "uq_product_media_checksum",
        "uq_product_media_primary",
        "ix_product_media_product_order",
        "ix_product_media_product_role",
        "ix_product_media_catalog",
        "ix_product_media_store",
        "ix_product_media_product",
    ):
